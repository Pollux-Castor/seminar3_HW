...
Для того чтобы скопировать удаленный репозиторий к себе на компьютер необходимо сделать следущее:

1. Создать новую папку и зайти в нее с помощью VS code или терминала.
2. В GitHub в выбранном репозитории нажимаем на кнопку _"Code"_(Код).
3. В открывшемся окне копируем адрес репозитория.
4. Возвращаемся в терминал и вводим команду:

         git clone <Repository address>

где вместо \<Repository address\> вводим ранее скопированный адрес репозитория.

## Делаем Pull request

Pull request (запрос на слияние) - это отправка запроса к автору кода на слияние сделанной вами работы над частью кода с основным кодом. Далее пошаговое описание процесса от создания ветки репозитория автора до отправки запроса:

1. Делаем _"fork"_ (вилку) репозитория выбранного автора.
2. Клонироуем (_"git clone"_) полученный репозиторий на локальную станцию.
3. В локальном репозитории создаем новую ветку (_"brench"_).
4. Переходим на новую ветку и в неё вносим свои изменения и дополнения.
5. Помним что каждое изменение должно фиксироваться (_"git add"_) с добавлением коммита (_"git commit"_).
6. После окончпния работы отправляем локальные изменения на внешний репозиторий (_"git push"_).
7. На сайте GitHub на странице репозитория с которым мы работали находим _"Pull request"_.
